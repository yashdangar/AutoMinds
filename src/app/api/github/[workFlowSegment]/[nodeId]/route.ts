...
export async function POST(
    ...
    const githubNode = await prisma.githubNode.update({
        ...
        data: {
            ...
            GithubTriggerRepoName : data.isTrigger ? data.repository.split("/")[1] : null ,
            GithubTriggerRepoOwner : data.isTrigger ? data.repository.split("/")[0] : null ,

            GithubActionType : !data.isTrigger ? data.action : null ,
            GithubActionRepoName : !data.isTrigger ? data.repository.split("/")[1] : null,
            GithubActionRepoOwner : !data.isTrigger ? data.repository.split("/")[0] : null,
